...
def main():
    ...
    dur = st.text_input('total duration')
    ...
    smean = st.text_input('mean of paket size transmitted by source')
    ...
    ct_srv_src = st.text_input('num of connections with same service & source addr in 100 conn')
    is_ftp_login = st.text_input('check on ftp session login by user')
    attack_cat = st.text_input('potential attack type')
